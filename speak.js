...
function addSpeakHandler(btn, word, langSel, defaultLang){
  btn.addEventListener("click",()=>{
    ...
    if(!hasSR()){ btn.nextElementSibling.textContent="⚠"; btn.nextElementSibling.title="Speech recognition not supported"; return; }
    stopSR();
    ...
    const status=btn.nextElementSibling; status.textContent="…"; status.title="Listening"; rec.start();
    rec.onresult=(ev)=>{
...
function render(view){
  ...
  visibleSections.forEach(sec=>{
    ...
    (sec.words||[]).forEach(word=>{
      ...
      addSpeakHandler(btn, word, document.getElementById("lang-select"), a.lang||"en-US");
    });
...
async function init(){
  try{
    ...
    app.a=a; app.totalWords=countTotalWords(a); app.done=0; app.view="all"; app.secIndex=0;
