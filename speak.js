...
function showErr(msg){const e=document.getElementById("err"); e.textContent=msg; e.hidden=false;}
function loadVoices(){function cache(){app.voices=window.speechSynthesis?speechSynthesis.getVoices():[];app.voiceByLang={};app.voices.forEach(v=>{(app.voiceByLang[v.lang]=app.voiceByLang[v.lang]||[]).push(v);});}
  try{cache(); if("onvoiceschanged" in speechSynthesis){speechSynthesis.onvoiceschanged=cache;}}catch(_){}}
function speakText(text,lang){try{ if(!window.speechSynthesis) return; const u=new SpeechSynthesisUtterance(text); u.lang=lang||"en-US"; const v=(app.voiceByLang[u.lang]||[])[0]; if(v) u.voice=v; speechSynthesis.cancel(); speechSynthesis.speak(u);}catch(_){}}
...
async function init(){
  try{
    loadVoices();
    const res=await fetch("speak-data.json",{cache:"no-store"});
    if(!res.ok) throw new Error("Cannot load speak-data.json");
    const data=await res.json();
    const a=(data.activities||[])[0]; if(!a) throw new Error("No activity in speak-data.json");

    const defaultLang=a.lang||"en-US";
    ...
    (data.languages||["en-US"]).forEach(l=>{const o=document.createElement("option"); o.value=l; o.textContent=l; if(l===defaultLang) o.selected=true; sel.appendChild(o);});
    ...
    // build columns
    const wrap=document.getElementById("columns"); wrap.innerHTML=""; app.totalWords=0; app.done=0; updateProgress();
    (a.hotspots||[]).forEach(sec=>{
      ...
      (sec.words||[]).forEach(word=>{
        ...
        btn.addEventListener("click",()=>{
          ...
          status.textContent="…"; status.title="Listening"; rec.start();
          rec.onresult=(ev)=>{
